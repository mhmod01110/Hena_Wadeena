...
const TourismPage = () => {
  ...
  const copy =
    language === "ar"
      ? {
          heroAlt: "السياحة في الوادي الجديد",
          badge: "السياحة والمجتمع",
          title: "السياحة والمجتمع",
          subtitle: "اكتشف المعالم السياحية، احجز مرشداً، أو ابحث عن سكن للطلاب",
          searchPlaceholder: "ابحث عن معالم، مرشدين، أو سكن...",
          tabs: { attractions: "المعالم", guides: "المرشدين", housing: "السكن" },
          ...
          perDay: "جنيه/يوم",
          book: "احجز",
          housingTitle: "سكن الطلاب والوافدين",
          housingSubtitle: "ابحث عن شقق وغرف مناسبة للطلاب والعاملين في الوادي الجديد",
          studentFriendly: "مناسب للطلاب",
          details: "تفاصيل",
          currencyPer: "جنيه/{{unit}}",
        }
      : {
          heroAlt: "Tourism in New Valley",
          badge: "Tourism and Community",
          title: "Tourism and Community",
          subtitle: "Explore landmarks, book a guide, or find accommodation for students",
          searchPlaceholder: "Search attractions, guides, or housing...",
          tabs: { attractions: "Attractions", guides: "Guides", housing: "Housing" },
          ...
          perDay: "EGP/day",
          book: "Book",
          housingTitle: "Student and Visitor Housing",
          housingSubtitle: "Find suitable apartments and rooms for students and workers in New Valley",
  ...
  return (
    <Layout>
      <PageTransition>
        <PageHero image={heroTourism} alt={copy.heroAlt}>
          <SR>
            ...
            <Tabs defaultValue="attractions" className="w-full">
              <SR>
                <TabsList className="grid w-full max-w-lg mx-auto grid-cols-3 mb-10 h-12 rounded-xl">
                  <TabsTrigger value="attractions" className="rounded-lg text-sm font-semibold">{copy.tabs.attractions}</TabsTrigger>
                  <TabsTrigger value="guides" className="rounded-lg text-sm font-semibold">{copy.tabs.guides}</TabsTrigger>
                  <TabsTrigger value="housing" className="rounded-lg text-sm font-semibold">{copy.tabs.housing}</TabsTrigger>
                </TabsList>
              </SR>
              ...
              <TabsContent value="attractions" className="space-y-8">
                {loading ? (
                  ...
                  <>
                    <SR>
                      <h3 className="text-2xl font-bold text-foreground mb-6">{copy.featured}</h3>
                    </SR>
                    <SR stagger>
                            ...
                            <CardContent className="p-6">
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <Button variant="outline" size="sm" className="hover:scale-[1.03] transition-transform" onClick={() => navigate(`/tourism/attraction/${attraction.id}`)}>
                                  {copy.more} <ArrowLeft className={`h-4 w-4 ${isRTL ? "mr-1" : "ml-1"}`} />
                                </Button>
                              </div>
                            </CardContent>
                          </Card>
                    ...
                    <SR>
                      <div className="flex items-center justify-between mb-6">
                        <h3 className="text-2xl font-bold text-foreground">{copy.allAttractions}</h3>
                        <Button variant="outline" className="hover:scale-[1.03] transition-transform" onClick={() => navigate("/tourism/attractions")}>{copy.viewMore}</Button>
                      </div>
                    </SR>
                    <SR stagger>
                          ...
                          <CardContent className="p-7">
                            <div className="flex items-center gap-4 mb-5">
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-1.5 text-accent">
                                  ...
                                  <span className="text-sm text-muted-foreground">({guide.reviews} {copy.reviews})</span>
                                </div>
                              </div>
                            </div>
                            <div className="space-y-3 mb-5">
                              <div>
                                <span className="text-sm text-muted-foreground">{copy.languages}:</span>
                                <div className="flex flex-wrap gap-1.5 mt-1.5">
                                  {guide.languages.map((lang) => (<Badge key={lang} variant="secondary" className="text-xs">{lang}</Badge>))}
                                </div>
                              </div>
                              <div>
                                <span className="text-sm text-muted-foreground">{copy.specialties}:</span>
                                <div className="flex flex-wrap gap-1.5 mt-1.5">
                            ...
                            <div className="flex items-center justify-between pt-5 border-t border-border">
                              ...
                              <Button className="hover:scale-[1.03] transition-transform" onClick={() => navigate(`/tourism/guide-booking/${guide.id}`)}>
                                <Calendar className={`h-4 w-4 ${isRTL ? "ml-2" : "mr-2"}`} />{copy.book}
                              </Button>
                            </div>
                          </CardContent>
                        </Card>
                ...
                <SR>
                  <div className="glass rounded-2xl p-5 mb-8">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-bold text-foreground text-lg">{copy.housingTitle}</h3>
                        <p className="text-sm text-muted-foreground">{copy.housingSubtitle}</p>
                      </div>
                    </div>
                  </div>
                </SR>
                        ...
                        <Card key={acc.id} className="border-border/50 hover:border-primary/40 hover-lift rounded-2xl">
                          <CardContent className="p-7">
                            <div className="flex items-start justify-between mb-4">
                              ...
                              {acc.for_students && (<Badge className="bg-accent text-accent-foreground">{copy.studentFriendly}</Badge>)}
                            </div>
                            ...
                            <div className="flex items-center justify-between pt-5 border-t border-border">
                              <div>
                                ...
                                <span className="text-sm text-muted-foreground ms-1">{copy.currencyPer.replace("{{unit}}", acc.price_unit)}</span>
                              </div>
                              <Button variant="outline" className="hover:scale-[1.03] transition-transform" onClick={() => navigate(`/tourism/accommodation/${acc.id}`)}>
                                <Phone className={`h-4 w-4 ${isRTL ? "ml-2" : "mr-2"}`} />{copy.details}
                              </Button>
                            </div>
                          </CardContent>
                        </Card>
