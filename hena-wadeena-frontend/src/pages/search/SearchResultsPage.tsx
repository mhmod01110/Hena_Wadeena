  ...
  const copy =
    language === "ar"
      ? {
          ...
          resultsFor: (count: number, value: string) => `${count} نتيجة لـ "${value}"`,
          types: {
            poi: "مكان",
      ...
      : {
          ...
          placeholder: "Search places, guides, investment opportunities...",
          noResults: "No results found. Try different keywords.",
          resultsFor: (count: number, value: string) => `${count} result(s) for "${value}"`,
          types: {
            poi: "Place",
      ...
      <PageTransition>
        <section className="relative py-14 md:py-20 overflow-hidden">
          ...
          <div className="container relative px-4 max-w-3xl">
            ...
            {query && <p className="text-sm text-muted-foreground mb-5">{copy.resultsFor(total, query)}</p>}
            ...
            <div className="space-y-4">
                        ...
                        <CardContent className="p-5 flex items-start gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <Badge className={typeColors[r.type] || ""} variant="outline">{copy.types[r.type as keyof typeof copy.types] || r.type}</Badge>
              ...
              {query && !loading && results.length === 0 && (
                <Card className="rounded-2xl"><CardContent className="p-14 text-center text-muted-foreground text-lg">{copy.noResults}</CardContent></Card>
              )}
            </div>
          </div>
        </section>
