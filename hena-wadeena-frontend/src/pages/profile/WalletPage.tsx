...
const WalletPage = () => {
  ...
  const handleTopup = async () => {
    ...
    if (!amount || amount <= 0) {
      toast.error(copy.enterValidAmount);
      return;
    }
    try {
          ...
          <div className="container relative px-4 max-w-2xl space-y-7">
                ...
                <CardContent className="p-10 text-center">
                  {loading ? (
                    ...
                    <>
                      ...
                      <p className="text-sm text-muted-foreground mb-2">{copy.currentBalance}</p>
                      <p className="text-6xl font-bold text-foreground">{wallet?.balance?.toLocaleString() ?? "0"}</p>
                      <p className="text-lg text-muted-foreground mt-2">{copy.currency}</p>
                      <div className="flex gap-3 justify-center mt-8">
                        <Button onClick={() => setShowTopup(!showTopup)} size="lg" className="h-14 px-8 rounded-xl hover:scale-[1.03] transition-transform">
                          <Plus className="h-5 w-5 ml-2" />{copy.topupWallet}
                        </Button>
                      </div>
                    </>
                  )}
              ...
              <SR>
                <Card className="border-primary/30 rounded-2xl">
                  <CardContent className="p-7 space-y-5">
                    <h3 className="font-bold text-lg">{copy.topupWallet}</h3>
                    ...
                    <Input type="number" placeholder={copy.customAmount} value={topupAmount} onChange={(e) => setTopupAmount(e.target.value)} className="h-12 rounded-xl" />
                    <div className="flex gap-3">
                      <Button onClick={handleTopup} className="flex-1 h-12 rounded-xl hover:scale-[1.02] transition-transform"><CreditCard className="h-5 w-5 ml-2" />{copy.topupNow}</Button>
                      <Button variant="outline" className="h-12 rounded-xl" onClick={() => setShowTopup(false)}>{copy.cancel}</Button>
                    </div>
                  </CardContent>
                </Card>
              </SR>
            ...
            <SR delay={200}>
              <Card className="rounded-2xl">
                <CardHeader><CardTitle className="text-xl">{copy.transactions}</CardTitle></CardHeader>
                <CardContent className="space-y-3">
                  {loading ? (
                    [1, 2, 3].map((i) => <Skeleton key={i} h="h-16" className="rounded-xl" />)
                  ) : transactions.length === 0 ? (
                    <p className="text-center text-muted-foreground py-10">{copy.noTransactions}</p>
                  ) : (
                    transactions.map((tx) => (
                      <div key={tx.id} className="flex items-center justify-between p-4 rounded-xl bg-muted/30 hover:bg-muted/50 transition-colors duration-200">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{new Date(tx.created_at).toLocaleDateString(language === "ar" ? "ar-EG" : "en-US")}</p>
                          </div>
                        </div>
                        <div className="text-left">
                          <p className={`font-bold text-lg ${tx.direction === "credit" ? "text-green-600" : "text-red-500"}`}>
                            {tx.direction === "credit" ? "+" : "-"}{tx.amount.toLocaleString()} {language === "ar" ? "جنيه" : "EGP"}
                          </p>
                          <Badge variant="outline" className="text-xs">{tx.status === "completed" ? copy.completed : copy.pending}</Badge>
                        </div>
                      </div>
                    ))
                  )}
