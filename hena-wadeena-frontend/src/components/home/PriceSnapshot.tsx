        ...
        <SR direction="up" className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-10">
          <div>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-chart-3/10 border border-chart-3/20 mb-5">
              ...
              <span className="text-sm font-semibold text-chart-3">{t("price.badge")}</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-bold text-foreground mb-2">{t("price.title")}</h2>
            <p className="text-lg text-muted-foreground">{t("price.subtitle")}</p>
          </div>
          <Link to="/marketplace">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      <th className="text-start py-5 px-6 text-sm font-semibold text-muted-foreground">{t("price.product")}</th>
                      <th className="text-start py-5 px-6 text-sm font-semibold text-muted-foreground">{t("price.price")}</th>
                      <th className="text-start py-5 px-6 text-sm font-semibold text-muted-foreground">{t("price.change")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {loading
                      ...
                      : priceData.map((item, index) => (
                          <tr key={item.name} className={`hover:bg-muted/20 transition-colors duration-200 ${index !== priceData.length - 1 ? "border-b border-border/50" : ""}`}>
                            ...
                            <td className="py-5 px-6">
                              ...
                              <span className="text-sm text-muted-foreground ms-1">{t("price.egpPerUnit", { unit: item.unit })}</span>
                            </td>
                            <td className="py-5 px-6">
                              <div
