...
export function ChatWidget() {
  ...
  const copy =
    language === "ar"
      ? {
          greeting: "مرحباً! أنا مساعدك الذكي في هنا وادينا. كيف أقدر أساعدك؟",
    ...
    <>
      ...
      {open && (
        <div className="fixed bottom-24 left-6 z-50 w-[360px] max-h-[500px] bg-background border border-border/50 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 max-h-[320px]" dir={direction}>
            {messages.map((msg, i) => (
          ...
          <div className="border-t p-3">
            <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex gap-2" dir={direction}>
              <Input
                value={input}
